create or replace function private.set_activity_interest(
  ...
  p_interest_mode text default 'individual_interest'
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  insert into public.session_interests (
    ...
    v_athlete_id,
    p_interest_mode,
    case when p_active then 'active' else 'cancelled' end,
  ...
  insert into public.audit_logs (
    ...
    auth.uid(),
    case when p_active then 'activity_interest.activated' else 'activity_interest.cancelled' end,
    'session_interest',
    v_interest_id,
    jsonb_build_object(
