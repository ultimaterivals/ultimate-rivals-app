-- Atomic UR Coins redemption for the Season 1 V1 Market.
-- Keeps ranking points and UR Coins fully separate and serializes balance/inventory checks.
...
begin
  ...
  if operation_id is null or char_length(trim(operation_id)) < 8 or char_length(operation_id) > 120 then
  ...
  if v_existing_source is not null then
    ...
    select
      ...
      mr.status,
      coalesce((
        select sum(case when tx.direction = 'credit' then tx.amount else -tx.amount end)::integer
  ...
  -- Serialize all wallet mutations for this athlete before calculating balance.
  perform 1
  ...
  if not v_offer.accepts_urc or v_offer.urc_amount is null or v_offer.urc_amount <= 0 then
  ...
  if v_offer.inventory_limit is not null and v_active_inventory >= v_offer.inventory_limit then
  ...
  if v_offer.per_athlete_limit is not null and v_athlete_redemptions >= v_offer.per_athlete_limit then
    raise exception 'Athlete redemption limit reached for this offer' using errcode = 'P0001';
  end if;

  select coalesce(sum(case when tx.direction = 'credit' then tx.amount else -tx.amount end), 0)::integer
    into v_balance
  from public.ur_coin_transactions tx
  where tx.athlete_id = v_athlete_id;
  ...
  insert into public.ur_coin_transactions (
    ...
    auth.uid(),
    jsonb_build_object('offer_id', v_offer.id, 'redemption_code', v_redemption_code)
  );
