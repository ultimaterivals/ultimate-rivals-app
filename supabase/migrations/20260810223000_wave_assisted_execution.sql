...
begin
  ...
  insert into public.audit_logs(
    ...
    'athlete_activation_wave',
    target_wave_id,
    jsonb_build_object('issued_count',invite_count),
    jsonb_build_object('reason',trim(target_reason),'source','admin_wave_execution')
  );
...
comment on function public.issue_athlete_activation_wave_invites(uuid,jsonb,text) is
  'Atomically issues or renews access invites for selected active unlinked wave members. Raw tokens are generated by the application and only hashes are persisted.';
