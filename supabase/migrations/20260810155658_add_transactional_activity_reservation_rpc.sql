...
begin
  ...
  where n.nspname = 'public'
    and r.relname = 'activity_reservations'
    and c.contype = 'c'
    and pg_get_constraintdef(c.oid) ilike '%eligibility%'
  limit 1;

  if v_constraint is not null then
    execute format('alter table public.activity_reservations drop constraint %I', v_constraint);
  end if;
end;
$$;

alter table public.activity_reservations
  add constraint activity_reservations_eligibility_check
  check (eligibility in ('pending','eligible','ineligible','override'));
...
begin
  ...
  if v_active_count >= v_opportunity.capacity_athletes then
    ...
    insert into public.audit_logs (
      ...
      v_reservation_id,
      jsonb_build_object('opportunity_id', p_opportunity_id, 'athlete_id', v_athlete_id, 'waitlist_position', v_waitlist_position),
  ...
  order by ap.ends_at asc nulls last, ap.starts_at asc nulls first, ap.created_at asc
  for update of ap
  limit 1;
  ...
  insert into public.commercial_credit_ledger (
    ...
    auth.uid(),
    jsonb_build_object('source', 'athlete_portal', 'eligibility', v_eligibility)
  );
...
create or replace function public.reserve_activity_opportunity(
  p_opportunity_id uuid,
  p_operation_id uuid
)
returns table (
...
set search_path = pg_catalog, public, private
as $$
  select * from private.reserve_activity_opportunity(p_opportunity_id, p_operation_id);
$$;

revoke all on function public.reserve_activity_opportunity(uuid, uuid) from public, anon;
grant execute on function public.reserve_activity_opportunity(uuid, uuid) to authenticated;
