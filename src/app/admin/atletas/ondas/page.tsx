...
const successMessages: Record<string, string> = {
  wave_created:
    "Onda criada. Agora selecione os atletas e registre o motivo da escolha.",
  member_selected:
    "Atleta incluído na onda sem alterar seu status institucional.",
  member_removed:
    "Atleta removido da onda. O histórico da decisão foi preservado.",
...
const errorMessages: Record<string, string> = {
  ...
  wrong_pole:
    "Este atleta pertence a outro polo e não pode entrar nesta onda regional.",
  ...
  reason_required:
    "Toda decisão de inclusão, remoção ou mudança de status exige motivo.",
...
export default async function AthleteActivationWavesPage({
  ...
  const candidates = snapshot.candidates.filter((candidate) => {
    ...
    if (selectedWave?.poleId && candidate.poleId !== selectedWave.poleId)
      return false;
    ...
    <div className="grid gap-8">
        ...
        <div className="flex gap-3">
          ...
          <div>
            <p className="font-bold text-sky-100">
              Seleção por decisão, não por dado inventado
            </p>
            <p className="mt-1 text-sm leading-6 text-zinc-400">
              A base atual não possui histórico esportivo completo o bastante
              para o sistema recomendar automaticamente os oito melhores
              candidatos. Por isso, a seleção desta primeira onda é uma decisão
              administrativa explícita e o motivo fica auditado. Ranking legado
              não vira ranking oficial do novo sistema.
            </p>
          </div>
        </div>
      </Card>
      ...
      <Card>
        <details open={snapshot.waves.length === 0}>
          <summary className="font-display cursor-pointer text-lg font-black uppercase">
            Criar nova onda
          </summary>
                ...
                {snapshot.candidates
                  ...
                  .filter(
                    (candidate, index, rows) =>
                      rows.findIndex(
                        (item) => item.poleId === candidate.poleId,
                      ) === index,
                  )
                  .map((candidate) => (
                    <option
                      key={candidate.poleId}
                      value={candidate.poleId ?? ""}
                    >
                      {candidate.poleName}
                    </option>
                  ))}
        ...
        <>
          <Card className="border-ur-gold/20">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  {selectedWave.poleName ?? "Todos os polos"} ·{" "}
                  {selectedWave.selectedCount}/{selectedWave.targetSize}{" "}
                  selecionados · {selectedWave.readyCount} prontos para piloto
                </p>
                {selectedWave.notes && (
            ...
            {waveOpen && (
              <details className="mt-5 border-t pt-4">
                ...
                <form
                  action={updateActivationWaveStatusAction}
          ...
          <section className="grid gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-500">
                Cada gate aponta a próxima ação real. A onda não executa essas
                ações sozinha.
              </p>
            </div>
                ...
                <Card key={member.athleteId}>
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {member.readyForPilot && (
                          <Badge>pronto para piloto</Badge>
                        )}
                      </div>
                      <p className="mt-2 text-xs text-zinc-500">
                        {member.poleName ?? "Sem polo"} · prioridade{" "}
                        {member.priority} · selecionado{" "}
                        {formatDate(member.selectedAt)}
                      </p>
                  ...
                  <div className="mt-5 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                    {member.gates.map((gate) => (
                      <div
                        key={gate.key}
                        className={`rounded-ur border p-3 ${gateClass(gate)}`}
                      >
                        <div className="flex items-center justify-between gap-2">
                  ...
                  {waveOpen && (
                    <details className="mt-4 border-t pt-3">
                      ...
                      <form
                        action={removeActivationWaveMemberAction}
            ...
            <section className="grid gap-4">
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-500">
                    {capacityRemaining} vaga(s) restante(s). O sistema não
                    recomenda automaticamente candidatos porque a evidência
                    histórica atual é insuficiente.
                  </p>
                </div>
              ...
              <div className="grid gap-3 xl:grid-cols-2">
                {candidates.map((candidate) => (
                  <Card key={candidate.athleteId}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-2 flex flex-wrap gap-2 text-[10px] font-bold uppercase">
                          ...
                          <span
                            className={
                              ["female", "male"].includes(candidate.gender)
                                ? "text-emerald-300"
                                : "text-sky-300"
                            }
                          >
                    ...
                    <form
                      action={selectActivationWaveMemberAction}
                      className="mt-4 grid gap-2 md:grid-cols-[1fr_90px_auto]"
                    >
                      <input
                        type="hidden"
                        name="waveId"
