    ...
    <div className="mx-auto grid max-w-7xl gap-10 pb-8">
        ...
        <div className="mt-3 grid gap-8 lg:grid-cols-[1.2fr_.8fr] lg:items-end">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-sm leading-6 text-zinc-400 sm:text-base">
              Escolha seu próximo jogo, acompanhe sua reserva e mantenha sua
              disponibilidade atualizada. Interesse, reserva, lista de espera,
      ...
      {viewer.isPreview ? (
        <Card className="border-ur-gold/35 bg-ur-gold/[.035]">
          <p className="text-ur-gold text-sm font-bold">
            Prévia somente leitura. Interesse, reserva, cancelamento e
            alterações de disponibilidade estão desabilitados.
          </p>
        </Card>
      ) : null}
      ...
      {next ? (
        <section className="grid gap-4" aria-labelledby="next-game-title">
          ...
          <div className="border-ur-gold/30 bg-ur-gold/[.03] grid gap-5 rounded-[1.8rem] border p-6 lg:grid-cols-[1fr_auto] lg:items-end">
            <div className="grid gap-3 text-sm text-zinc-300 sm:grid-cols-2">
              {next.startsAt ? (
                <p className="flex items-center gap-2">
                  <CalendarClock
                    ...
                    aria-hidden="true"
                  />
                  {dateFormatter.format(new Date(next.startsAt))}
                </p>
              ) : null}
              <p className="flex items-center gap-2">
                <MapPin size={17} className="text-ur-gold" aria-hidden="true" />
                {[next.poleName, next.venueName].filter(Boolean).join(" · ") ||
              ...
              <p className="text-ur-gold flex items-center gap-2 font-black">
                <Clock3 size={17} aria-hidden="true" />
                {reservationLabel(next.personalReservationStatus) ??
        ...
        <section className="rounded-[1.75rem] border border-white/8 bg-white/[.02] p-6">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">
            Isso não significa que não existam oportunidades. Consulte as
            atividades abertas abaixo e acompanhe o estado real de cada
            inscrição.
          </p>
        </section>
      )}
      ...
      >
        <div className="flex items-end justify-between gap-4">
          ...
          <span className="text-xs font-black text-zinc-600">
            {opportunities.length} publicada(s)
          </span>
        </div>
        ...
        {!snapshot.identity ? (
          <Card>
            ...
            <p className="mt-2 text-sm text-zinc-500">
              As ações de jogo ficam disponíveis somente depois do vínculo com
              seu cadastro esportivo oficial.
            </p>
          </Card>
          ...
          <Card>
            ...
            <p className="mt-2 text-sm leading-6 text-zinc-500">
              Quando uma nova atividade for publicada para sua jornada, ela
              aparecerá aqui. Sua disponibilidade ajuda o planejamento, mas não
              cria vaga nem reserva automática.
            </p>
          </Card>
        )}
        ...
        {!creditsKnown ? (
          <p className="text-xs leading-5 text-zinc-600">
            A fonte de créditos está indisponível. O App não assume saldo zero e
            o estado econômico não é usado como informação confiável até a fonte
            responder.
          </p>
        ) : null}
      </section>
      ...
      >
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-500">
            Registre suas janelas recorrentes. Disponibilidade é preferência
            operacional: não garante sessão, não reserva vaga, não consome
            crédito e não confirma participação.
          </p>
        </div>
        ...
        {!availability.athleteId ? (
          <Card>
            <p className="text-sm text-zinc-500">
              A disponibilidade será liberada depois que sua conta estiver
              vinculada ao cadastro oficial de atleta.
            </p>
          </Card>
        ) : viewer.isPreview ? (
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        ...
        >
          ...
          <p className="mt-2 text-sm leading-6 text-zinc-500">
            Veja somente locais associados a oportunidades oficiais e mídia
            publicada.
          </p>
        </Link>
        <Link
        ...
        >
          ...
          <p className="mt-2 text-sm leading-6 text-zinc-500">
            Resultados e histórico homologado continuam separados da jornada de
            entrada em quadra.
          </p>
        </Link>
      </section>
