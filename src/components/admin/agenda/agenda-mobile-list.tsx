        ...
        return (
          <section key={day.date} className="grid gap-2">
            <div className="flex items-baseline justify-between gap-3">
              <h2
                className={`font-display text-lg font-black uppercase ${day.isToday ? "text-ur-gold" : ""}`}
              >
                {day.weekday} · {day.shortLabel}
              </h2>
              {day.isToday && (
                <span className="text-ur-gold text-xs font-bold uppercase">
                  Hoje
                </span>
              )}
            </div>
            {events.length === 0 ? (
              <Card className="py-4 text-sm text-zinc-600">
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-xs text-zinc-500">
                        ...
                        {event.reservedCount > 0 && (
                          <span className="text-ur-gold flex items-center gap-1">
                            <TicketCheck size={13} aria-hidden="true" />
                            {event.reservedCount} reservas
                          </span>
                        )}
                      </div>
                    </div>
