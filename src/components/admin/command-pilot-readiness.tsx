...
function gateClass(gate: PilotReadinessGate) {
  if (gate.state === "ready") return "border-emerald-500/25 bg-emerald-500/5";
  if (gate.state === "attention") return "border-amber-500/25 bg-amber-500/5";
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">
            ...
            infraestrutura, sessão, motor esportivo e fontes críticas estão
            realmente prontos para uma execução de ponta a ponta.
          </p>
        </div>
      ...
      {snapshot.currentWave && (
        <div className="rounded-ur mt-5 border px-4 py-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 font-bold text-white">
                {snapshot.currentWave.name}
              </p>
            </div>
            <p className="text-xs text-zinc-500">
              {snapshot.currentWave.selectedCount}/
              {snapshot.currentWave.targetSize} selecionados ·{" "}
              {snapshot.currentWave.readyCount} prontos
            </p>
          </div>
        </div>
      )}
      ...
      <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {snapshot.gates.map((gate) => (
          <Link key={gate.key} href={gate.href} className="group block">
            <div
              className={`rounded-ur h-full border p-4 transition group-hover:-translate-y-0.5 ${gateClass(gate)}`}
            >
