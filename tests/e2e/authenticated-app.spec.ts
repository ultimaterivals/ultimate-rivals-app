...
test("athlete opens preserved Player Hub and core destinations", async ({ page }) => {
...
test("admin Preview renders athlete App read-only without replacing admin Auth", async ({ page }) => {
  ...
  await expect(page.getByText(/Prévia do Atleta · somente leitura/i)).toBeVisible();
  await expect(page.getByRole("button", { name: "Voltar ao Command" })).toBeVisible();
